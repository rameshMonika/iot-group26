...
#include <RadioLib.h>     // Library for radio communication
#include <AES.h>          // AES encryption/decryption
#include <Crypto.h>       // Crypto library used by AES
#include <Wire.h>         // I2C communication
#include "SSD1306Wire.h"  // OLED display library

// OLED display object (address 0x3C using SDA and SCL pins)
SSD1306Wire display(0x3C, SDA, SCL);

// Create a LoRa radio object with specified pins
SX1280 radio = new Module(RADIO_CS_PIN, RADIO_DIO1_PIN, RADIO_RST_PIN, RADIO_BUSY_PIN);
String nodeID = "Node_D";  // Set unique IDs for each receiver node
volatile bool receivedFlag = false; // Flag to indicate packet reception
String payload; // Storage for received packet data

// AES-128 encryption key (shared by all nodes in the network)
byte aesKey[16] = {
...
// Time slot configuration for sending (to avoid collisions)
const int mySlot = 2;         // Set to 0,1,2 for different nodes, this node's slot index is index 2
const int slotDelay = 5000;   // Duration of each slot in milliseconds

// Structure to track last message timestamps from devices to prevent duplicates
struct DeviceRecord {
  String id;
  String lastTimestamp;
};
DeviceRecord knownDevices[5]; // Store recent devices (up to 5)

void setFlag() {
  receivedFlag = true;  // Set flag when packet is received
}
...
void setup() {
  ...
  digitalWrite(RADIO_TCXO_ENABLE, HIGH);  // Enable TCXO if defined
#endif
...
#ifdef RADIO_RX_PIN
  radio.setRfSwitchPins(RADIO_RX_PIN, RADIO_TX_PIN);  // RF switch setup
#endif
  ...
  for (int i = 0; i < 5; i++) {
    if (knownDevices[i].id == devID) {
      if (knownDevices[i].lastTimestamp == ts) {
        ...
        knownDevices[i].lastTimestamp = ts; // Update timestamp
        return false;
      }
    }
  }
...
String decryptAES(const String& encryptedHex) {
  ...
  return String(output);  // Return decrypted plaintext
}
...
void loop() {
  if (receivedFlag) {
    ...
    delay(5000);  // Cooldown before listening again

    radio.startReceive();  // Resume listening for packets
  }
}